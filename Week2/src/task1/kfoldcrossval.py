...
def main(args):
    ...
    # Strategy B (sequential folds)

    if strategy == "b":
        print("Using sequential fold assignment...")
        folds = np.array_split(indices, 4)

    elif strategy == "c":

        # (random folds)
        print("Shuffling dataset for random fold assignment...")
        np.random.shuffle(indices)
        folds = np.array_split(indices, 4)

    else:
        raise ValueError("Invalid strategy. Choose 'b' or 'c'.")
    ...
    for fold in range(4):
        ...
        print(f"\n{'#'*40}")
        print(f"Running Fold {fold+1}/4")
        print(f"{'#'*40}")

        train_idx = folds[fold]
        test_idx = np.concatenate([folds[i] for i in range(4) if i != fold])

        train_dataset = Subset(full_dataset, train_idx)
        eval_dataset = Subset(full_dataset, test_idx)
        ...
        if log_wandb:
            wandb.init(
                project="C6-Week2",
                entity="c6-team2",
                name=f"Finetune-{model_name}-fold{fold}",
                config=args,
            )
    ...
    print("\nK-Fold Results")
    print("mAP per fold:", fold_maps)
    print("Average mAP:", np.mean(fold_maps))
