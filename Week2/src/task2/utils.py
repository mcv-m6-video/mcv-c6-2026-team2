...
def load_detections(file_path):
    """
    Loads detections extracted from Task 1 with Faster R-CNN.
    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            left, top, w, h = (float(parts[2]), float(parts[3]),
...
def convert_xml_to_mot(xml_path, output_txt_path):
    ...
    for track in root.findall('track'):
        ...
        for box in track.findall('box'):
            ...
            lines.append(f"{frame},{track_id},{xtl:.2f},{ytl:.2f},{w:.2f},"
                         f"{h:.2f},1,-1,-1,-1\n")
...
def prepare_trackeval_folders(base_trackeval_path, results_path, gt_path,
    ...
    tracker_data_path = os.path.join(
        data_path, "trackers", "mot_challenge", f"{benchmark}-{split}",
        tracker_name, "data")
    ...
    shutil.copy(results_path, os.path.join(
        tracker_data_path, f"{sequence}.txt"))
    # GT file MUST be named gt.txt
    shutil.copy(gt_path, os.path.join(gt_base_path, sequence, "gt", "gt.txt"))

    print(f"TrackEval structure ready for benchmark '{benchmark}' and tracker "
          f"'{tracker_name}'")
