...
def evaluate_car_state(car: Car, curr_cam: Camera, t_manager: TrackManager, border_threshold_meters: float =5.0, overlap_threshold: float =0.8):
    ...
    for overlap_cam in curr_cam.overlapping_cameras:
        if car in overlap_cam:
            intersection_area = car.gps_bbox[-1].intersection(overlap_cam.gps_polygon).area
            ...
            if car_area > 0:
                ...
                if overlap_ratio >= overlap_threshold:
                    global_id = t_manager.local_to_global[(curr_cam.camera_idx, car.car_id)]
    ...
    dist_to_border = curr_cam.gps_polygon.exterior.distance(car.gps_bbox[-1])

    if dist_to_border < border_threshold_meters:
        return {
            "action": "ADJACENCY",
...
def main(args):
    ...
    local_cars_registry: dict[int, dict[int, Car]] = {i: {} for i in range(len(cam_list))}
    overlap_queue = {}
    adjacency_queue = {}

    while True:
        active_cars_current_frame: dict[int, list[Car]] = {i: [] for i in range(len(cam_list))}
        ...
        for cam_idx in range(len(cam_list)):
            ...
            for d in dets:
                ...
                car_id = int(car_id)
                bbox = np.array([float(xleft), float(ytop), float(xright), float(ybottom)], dtype=np.float32)
        ...
        for queue in [overlap_queue, adjacency_queue]:
            ...
            for key, queue_info in queue.items():
                ...
                for target_cam in queue_info["targets"]:
                    target_active_cars: list[Car] = active_cars_current_frame[target_cam.camera_idx]
                    matched = False

                    for target_car in target_active_cars:
                        global_id_source: int = t_manager.local_to_global[(source_cam_idx, source_car_id)]
                        global_id_target: int = t_manager.local_to_global[(target_cam.camera_idx, target_car.car_id)]
                        ...
                        if source_car == target_car:
                            t_manager.link_cars(source_cam_idx, source_car_id, target_cam.camera_idx, target_car.car_id)
                            matched = True
                            break
                ...
                if frame_idx >= queue_info["expires_at"] or len(queue_info["targets"]) == 0:
