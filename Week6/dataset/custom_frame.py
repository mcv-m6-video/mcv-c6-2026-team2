# New frame file because why different classes if it is only 1 dataset.
# And why is the randomness inherent to the dataset instead of an outside factor?
# Also, the length of the dataset should depend on the data, not a hyperparameter, that's what number of epochs is for.
# This dataset should always return the same whether it is train, val or test.
...
class ActionSpotDataset(Dataset):
    def __init__(
            self,
            class_dict: dict[str, int],                                 # dict of class names to idx
            game_file: str,                                             # path to videos json
            frame_dir: str,                                             # path to frames
            store_dir: str,                                             # path to store files (with frames path and labels per clip)
            store_mode: Literal['store', 'load'],                       # 'store' or 'load'
            clip_len: int,                                              # Number of frames per clip
            stride: int = 1,                                            # Downsample frame rate
            overlap: float = 1.0,                                       # Overlap between clips (in proportion to clip_len)
            pad_len: int = DEFAULT_PAD_LEN,                             # Number of frames to pad the start
                                                                        # and end of videos
            dataset: Literal['soccrenetball'] = 'soccernetball',        # Dataset name
            labels_dir: str | None = None,                              # Directory with labels for SoccerNetBall ## Isn't this redundant if we have game_file???
            task: Literal['classification', 'spotting'] = 'spotting'    # Classification or localization
    ):
        # Comprobations
        ...
        self._split = os.path.splitext(os.path.basename(game_file))[0]
        self._store_dir = store_dir
        self._store_mode = store_mode
        self._clip_len = clip_len
        self._stride = stride
        self._clip_sampling_step = int((1 - overlap) * clip_len * stride)
    ...
    def print_info(self):
        ...
        print('{} : {} videos, {} frames'.format(
            self._game_file, len(self._games), num_frames))
